...
from datapack import Command, EntitySelector, Function, FunctionTag, INbt, List, Position, Str, Value
...
_general += Command.Tag.Add(EntitySelector.S(), _summoned_tag)
_general += Command.Tag.Remove(EntitySelector.S(), _on_summon_tag)
...
def Summon(type: str, pos: Position.IPosition, **nbt: Value[INbt]):
  ...
  f += EntitySelector.E(tag=_on_summon_tag, limit=1).As().At(EntitySelector.S()) + summon.call()
  return f.call()
...
_tick += EntitySelector.E(tag='!'+_summoned_tag).As().At(EntitySelector.S()) + summon.call()
